...
Confirm cloned DEV and TEST environments.
Confirm solution import/export route.
Confirm plugin deployment approach.
Confirm SharePoint access and document library/template configuration.
Validate Solution 07 API/plugin dependencies.
Validate Solution 08 permit fields and Generate Permit flow.
Complete mapping for expanded Search API response.
Complete and test Endorse Permit plugin.
Confirm QR/barcode final design with frontend/security.
Agree internal APHA digital permit process and transition journey.
Contact / ownership
Area	Contact / Team
